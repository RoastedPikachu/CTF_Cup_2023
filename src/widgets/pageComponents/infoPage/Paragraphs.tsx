...
const Paragraphs: React.FC<ParagraphsProps> = ({ isMobile }) => {
  ...
  return (
    <section className="mt-[120px] mlarge:mt-[100px] mmedium:mt-[80px] msmall:mt-[60px] pb-[100px] px-[10%] deskWide:px-[0px] mlarge:px-[5%] w-full">
      <img
        src="/static/infoPage/text/FAQText.svg"
        alt="FAQ"
        className="w-[25%] mlarge:w-[50%] mmedium:w-[55%] pb-[40px] mlarge:pb-[20px] mmedium:pb-[0px]"
      />

      {paragraphs.map((paragraph: Paragraph) => (
        <div
          key={paragraph.id}
          className="mt-[30px] mlarge:mt-[50px] pb-[20px] h-auto border-b-2 border-[#bf5af2]"
        >
          <div className="flex justify-between items-center w-[calc(100%-12.5px)] h-[50px]">
            <h3 className='w-[70%] mlarge:w-full text-[#ffffff] text-[1.5rem] mlarge:text-[1.25rem] mmedium:text-[1.125rem] font-["Good_Timing"] font-bold'>
              {paragraph.title}
            </h3>
            ...
            {!isMobile && (
              <button
                ...
                onClick={() => {
                  paragraph.isOpen = !paragraph.isOpen;
                  setParagraphs([...paragraphs]);
                }}
              >
                <img
                  src="/static/infoPage/icon/XmarkIcon.svg"
                  alt="Иконка открытия и закрытия"
                  ref={nodeRef}
                  className={`w-full h-full ${
                    paragraph.isOpen ? "rotate-0" : "rotate-45"
          ...
          {(paragraph.isOpen || isMobile) && (
            <p
              dangerouslySetInnerHTML={{ __html: paragraph.text }}
              className="mt-[30px] pb-[20px] text-[#ffffff] text-[1rem] mmedium:text-[0.875rem]"
            ></p>
          )}
        </div>
      ))}
